...
function getUserCards() {
    ...
    const dueNowRegular = userCustomFlashcards.filter(card =>
        card.isUserCard && !card.isDifficult && card.dueDate && isCardDue(card) && !card.movedFromDifficultAt
    ...
    const dueFromDifficultUser = userCustomFlashcards.filter(card =>
        card.isUserCard && !card.isDifficult && card.dueDate && isCardDue(card) && typeof card.movedFromDifficultAt === 'number'
    );

    // Senza scadenza
    const noDue = userCustomFlashcards.filter(card =>
        card.isUserCard && !card.isDifficult && !card.dueDate
    );
...
function getDefaultCards() {
    return dirittoPrivatoFlashcards;
}
...
function getAllFlashcards() {
    return [...dirittoPrivatoFlashcards, ...userCustomFlashcards];
}
    ...
    <div class="modal-body">
      <div class="help-section">
        ...
        <ul>
          ...
          <li><strong>Tasti rapidi</strong>: <kbd>Spazio</kbd> flip/avanti; <kbd>1–6</kbd> intervalli; <kbd>ESC</kbd> indietro; <kbd>Tab</kbd> avanti nelle modalità in scadenza.</li>
        </ul>
      </div>
